    ...
    for part in parts:
        if part.startswith("<tag"):
            ...
            if current_tag:
                ...
                part
            ):  # Preserve whitespace-only parts too — dropping them merges words
                nr = ET.Element(f"{{{NS[namespace_prefix]}}}r")
...
def _fix_run_boundaries(runs: list[ET.Element], t_tag: str) -> None:
    ...
    for i in range(len(runs) - 1):
        ...
        if _needs_space_between(left_text, right_text):
            ...
            t_left.set("{http://www.w3.org/XML/1998/namespace}space", "preserve")


def preserve_xml_declaration(
    modified_root: ET.Element, original_buffer: bytes
) -> bytes:
    ...
    3. Merge any xmlns declarations ET hoisted to root that weren't in the
       original (e.g., inline xmlns:r on <legacyDrawing>).
    4. Replace ET's generated root opening tag with the merged original.
    ...
    raw = (
        original_buffer
        if isinstance(original_buffer, bytes)
        else original_buffer.encode("utf-8")
    )
    ...
    for idx, c in enumerate(et_xml):
        if c == ">":
            et_root_end = idx + 1
            break
    et_root_open = et_xml[:et_root_end]
    et_inner = et_xml[et_root_end:]

    # 5. Merge xmlns declarations ET hoisted to root that are missing from original
    # ET hoists inline xmlns (e.g., xmlns:r on <legacyDrawing>) to root during
    # serialization. If we blindly replace ET's root with the original, those
    # prefixes become unbound in child elements → XML parse error / corruption.
    et_ns_decls = set(re.findall(r'xmlns(?::\w+)?="[^"]*"', et_root_open))
    orig_ns_decls = set(re.findall(r'xmlns(?::\w+)?="[^"]*"', orig_root_open))
    missing_ns = et_ns_decls - orig_ns_decls

    if missing_ns:
        # Insert missing xmlns into original root tag (before the closing >)
        extra = " ".join(sorted(missing_ns))
        if orig_root_open.endswith("/>"):
            orig_root_open = orig_root_open[:-2] + " " + extra + "/>"
        else:
            orig_root_open = orig_root_open[:-1] + " " + extra + ">"

    # 6. Reassemble: declaration + original root tag + ET's inner content
    ...
    # 7. Normalize self-closing tags: Python ET always serializes as " />"
