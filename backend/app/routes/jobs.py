...
@router.get("/jobs")
async def list_all_jobs(request: Request, current_user: dict = Depends(get_current_user)):
    """List recent translation jobs.

    Returns:
        List of recent jobs with status, filename, etc.
    """
    jobs = await list_jobs(owner_id=current_user.get("id"))
    return [
        {
            "id": j.id,
            "filename": j.filename,
...
async def get_job_detail(request: Request, job_id: str, current_user: dict = Depends(get_current_user)):
