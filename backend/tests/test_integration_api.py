...
def app_and_client(tmp_dirs, user_a, user_b):
    ...
    # Patch db at the module level — this makes ALL imports of `db` use the mock
...
class TestAuthProtection:
    ...
    def test_upload_requires_auth(self, app_and_client):
        ...
        resp = client.post("/api/upload", files={"file": ("test.docx", b"data", "application/octet-stream")})
...
class TestAuthEndpoints:
    ...
    def test_login_correct_credentials(self, app_and_client, user_a):
        ...
        resp = client.post("/api/auth/login", data={"username": "alice", "password": "alice_pass"})
    ...
    def test_login_wrong_password(self, app_and_client, user_a):
        ...
        resp = client.post("/api/auth/login", data={"username": "alice", "password": "wrong"})
        assert resp.status_code == 400
    ...
    def test_login_nonexistent_user(self, app_and_client):
        ...
        resp = client.post("/api/auth/login", data={"username": "ghost", "password": "pass"})
        assert resp.status_code == 400
...
class TestUploadWithAuth:
    ...
    def test_upload_sanitizes_filename(self, app_and_client, auth_headers_a, tmp_dirs):
        ...
        resp = client.post(
            "/api/upload",
            headers=auth_headers_a,
            files={"file": ("../../../etc/passwd.docx", b"data", "application/octet-stream")},
        )
        data = resp.json()
        if "filename" in data:
