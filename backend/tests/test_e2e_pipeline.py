...
from app.agent.extractor import extract_xlsx
from app.agent.reconstructor import reconstruct_xlsx

SAMPLES_DIR = os.path.join(os.path.dirname(__file__), "../../samples")
TEST_XLSX = os.path.join(
    SAMPLES_DIR, "【基本設計書】依頼届出_雇保給付金申請_高年齢_画面設計_202510.xlsx"
)
...
def test_full_pipeline_xlsx():
    ...
    for seg in segments:
        text = seg["text"]
        translated_segments.append({"text": text, "translated_text": text + " _vi"})
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with zipfile.ZipFile(output_path, "r") as zout:
            ...
            assert (
                "xl/drawings/drawing1.xml" in zout.namelist()
                or "xl/drawings/vmlDrawing1.vml" in zout.namelist()
            )
