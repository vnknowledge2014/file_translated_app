...
def test_zero_corruption_clone():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with (
            zipfile.ZipFile(TEST_XLSX, "r") as zin,
        ...
        assert size_diff < 0.05, (
            f"Clone size massively drifted: {orig_size} vs {clone_size}"
        )
...
def test_tag_validation_logic():
    ...
    assert count_tags(bad_translation) != orig_tags, (
        "Bad translation should be caught by RALPH"
    )
